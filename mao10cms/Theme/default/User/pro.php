	...
	<div class="container">
		<div class="row">
			<div class="col-lg-12" id="pro-all-list">
				...
				<?php $cart = M('action')->where("user_id='".mc_user_id()."' AND action_key IN ('wait_send','wait_cofirm','wait_finished') AND date='".$trade['date']."'")->order('id desc')->select(); ?>
				...
				<div class="panel-footer">
					<?php
						echo M('action')->where("action_key IN ('address_wait_send','address_wait_cofirm','address_wait_finished') AND date='".$trade['date']."'")->getField('action_value');
					?>
				</div>
				</div>
				<?php endforeach; ?>
