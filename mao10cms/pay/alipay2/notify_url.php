...
if($verify_result) {//验证成功
	...
	//该判断表示买家已经确认收货，这笔交易完成
        ...
        //调试用，写文本函数记录程序运行情况是否正常
        //logResult("这里写入想要调试的代码变量值，或其他运行的结果记录");
        $date = M('action')->where("user_id='$user_id' AND action_key='trade_wait_cofirm' AND action_value='$out_trade_no'")->getField('date');
        $cart = M('action')->where("user_id='$user_id' AND action_key='wait_cofirm' AND date='$date'")->order('id desc')->select();
        $action['action_key'] = 'wait_finished';
        foreach($cart as $val) :
        M('action')->where("id='".$val['id']."'")->save($action);
        endforeach;
        $action['action_key'] = 'address_wait_finished';
        M('action')->where("user_id='$user_id' AND action_key='address_wait_cofirm' AND date='$date'")->save($action);
        $action['action_key'] = 'trade_wait_finished';
        M('action')->where("user_id='$user_id' AND action_key='trade_wait_cofirm' AND date='$date'")->save($action);
    }
    else {
		//其他状态判断
        echo "success";
